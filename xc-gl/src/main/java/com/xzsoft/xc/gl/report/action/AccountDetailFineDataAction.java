...
/**
 * @fileName AccountDetailFineDataAction
 * @desc 总账明细自定义数据集
 * @author tangxl
 * @date 2016年3月23日
 */
@SuppressWarnings("serial")
public class AccountDetailFineDataAction extends AbstractTableData {
	 // 列名数组，保存程序数据集所有列名  
    ...
    public AccountDetailFineDataAction() {
		//构造函数初始化报表参数
		...
		this.parameters = new Parameter[]{new Parameter("isFirst"),
				...
				new Parameter("CCID"),new Parameter("DEFAULT_CURRENCY"),
				new Parameter("ACC_CATEGORY"),new Parameter("isAccount")
		};
	...
	public void fecthReportData(){
		...
		if(parameters!=null && parameters.length>0){
			...
			String endAccCode = map.get("END_ACCOUNT");
			if(endAccCode!=null && !"0".equals(endAccCode)){
				...
				map.put("END_ACCOUNT", endAccCode);
			}
			try {
