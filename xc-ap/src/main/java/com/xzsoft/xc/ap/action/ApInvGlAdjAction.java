 ...
 */
@Controller
@RequestMapping("/apInvGlAdjAction.do")
public class ApInvGlAdjAction {
	private static Logger log = Logger.getLogger(ApInvGlAdjAction.class.getName());
	@Resource
	ApInvGlAdjService service;
	/*
	 * 保存调整单
	 */
	...
	public void deleteAdj(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			jo = service.deleteAdj(rows, apInfo,language);
		} catch (Exception e) {
			jo.put("flag", "1");
			jo.put("msg", e.getMessage());	
		}
	...
	public void checkArVoucher(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			String apInfo = request.getParameter("apInfo");
			jo  = service.checkArVoucher(apInfo,language);
		} catch (Exception e) {
			jo.put("flag", "1");
			jo.put("msg", e.getMessage());	
		}
	...
	public void cancelCheckArVoucher(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			String apInfo = request.getParameter("apInfo");
			jo = service.cancelCheckArVoucher(apInfo,language);
		} catch (Exception e) {
			jo.put("flag", "1");
			jo.put("msg", e.getMessage());	
		}
